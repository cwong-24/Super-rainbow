...
public class Panel extends JPanel implements KeyListener, Runnable {
    ...
    public void setBackground(Graphics g) {
        ...
        if(pop.getLevel() == 1) {
            try{
                if(pop.isLive())
                    img = ImageIO.read(new File("resources/image/Background1.bmp"));
                else
                    img = ImageIO.read(new File("resources/image/Title.bmp"));
        ...
        if(pop.getLevel() == 2) {
            try{
                if(pop.isLive())
                    img = ImageIO.read(new File("resources/image/Background2.bmp"));
                else
                    img = ImageIO.read(new File("resources/image/Title.gif"));
        ...
        if(biglegs2.size() == 0) {
            ...
            try{
                img = ImageIO.read(new File("resources/image/Congratulation.gif"));
                g.drawImage(img, 0, 0, null);
            }catch(IOException ex){
                System.out.println(ex.getMessage());
            }
